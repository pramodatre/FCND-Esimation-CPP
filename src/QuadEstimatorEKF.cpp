...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  // update velocites
  predictedState(3) = curState(3) + accelInertial.x * dt;
  predictedState(4) = curState(4) + accelInertial.y * dt;
  predictedState(5) = curState(5) + (accelInertial.z - CONST_GRAVITY) * dt;
  // retain yaw as-is (not updated)
